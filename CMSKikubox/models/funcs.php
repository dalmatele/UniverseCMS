...
{
	$directory = "";
        //Đường dẫn cho CMS - ducla
	$pages = glob($directory . "*.php");
	//print each file name
	foreach ($pages as $page){
		$row[$page] = $page;
	}
        //Thêm các file trong View của hệ thống CMS
        $cmsDirectory = "view/Admin/";
        $cmsPages = glob($cmsDirectory . "*.php");
        foreach ($cmsPages as $page){
            $index = strrpos($page, "/");
            $length = strlen($page);
            $page = substr($page, $index + 1, $length - $index);
            $row[$page] = $page;
	}
	return $row;
}
